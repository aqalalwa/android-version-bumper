...
Toolkit.run(async (tools): Promise<void> => {
  try {
    ...
    const releaseFlavor = getReleaseFlavor(tools);
    const versionFileExists = await doesVersionPropertiesExist(fs, releaseFlavor);

    let build: Build;

    if (versionFileExists) {
      const existingVersion = await getVersionProperties(tools, releaseFlavor);
    ...
    await setVersionProperties(fs, tools, build.version, releaseFlavor);
    await setGitIdentity(tools);
    await createCommit(tools, message, releaseFlavor);
